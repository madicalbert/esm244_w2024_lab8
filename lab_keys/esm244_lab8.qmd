...
## where(is.numeric) function  will select all of the columns where this is true, so take all columns that are numeric values and pivot it longer

ggplot(hdi_data_long, aes(x = value)) +
  geom_histogram() +
  facet_wrap(~ name, scales = 'free_x')
## (scales = 'free_x') allows each axis in the subplots to look nice 


## Notice some of the skews, we don't need to be too worried about this, but some big outliers could be problematic - could do a log transorm, but we don't need to worry about it. 
```
...
hdi_data <- hdi_data_raw %>%
  ...
  ## Turn the hdicode variable into a factor by imposing some levels on it so that they appear in a logical order for us. 
...
### we are trying to be more consistent with the HDI approach. 

hdi_pca <- hdi_data %>% 
  select(where(is.numeric)) %>%
### we need just the numeric values, it also can not deal with missing values (we do not have missing values, so we are okay, but not in the post lab or HW 4).
### use select and where function again to get just the numeric values
  # select(-iso3, -country, -hdicode)) %>%
  # select(ends_with('2021'), lat) %>%
  prcomp(scale = TRUE)
### all of our values on are differnt scales (2-25, 8-50, etc.) so we want to rescale so that they are recentered (mean = 0, SD = 1).
### why is it a list of 5? Use "str(hdi_pca)" which is the structure function, 


### can see the SD, rotation is like the variance, center is the amount adjusted, scale is how much it rescaled it, x is the new dimensions in the pca space 
```
...
hdi_pca$rotation
### "hdi_pca$rotation" shows you the 9 principle components for each variable
```
...
### axis is the row.names from the previous argument, so the df with the rotation columnn in the hdi_pca
### the "." = data.frame(hdi_pca$rotation)

new_pts_df <- data.frame(hdi_pca$x)
### the x values is the transformed values in the new coordinate system 
### now we have the points and the arrows
...
ggplot() +
  ...
  geom_segment(data = loadings_df,
               x = 0, y = 0, ### this is where the values start
               aes(xend = PC1, ### how much does this contribute to PC1
                   yend = PC2, ### how much does this contribute to PC2
                   group = axis),
               arrow = arrow(length = unit(.25, 'cm'))) + ### adding the arrows
  geom_text(data = loadings_df, ### add the end of each arrow, add a label from the axis column
            aes(x = PC1, y = PC2, label = axis), vjust = 0, ### point is bottom of text
            nudge_y = .01) + ### nudge up so that label is not on end of arrow 
  theme_minimal()
```
...
```{r}
autoplot(hdi_pca,
     	data = hdi_data, ### can grab the other data like names from this df
     	loadings = TRUE, 
     	colour = 'hdicode', ### point color based on this variable 
...
### ggrepel() could work to prevent the overlapping labels, may not work with autoplot 
```
